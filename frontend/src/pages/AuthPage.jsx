...
const AuthPage = () => {
  ...
  useEffect(() => {
    const handleToken = async () => {
      ...
      if (token) {
        // Heuristic: JWTs contain periods (header.payload.signature)
        // Password reset tokens (UUIDs) do not.
        if (token.includes(".")) {
          // --- HANDLE GOOGLE LOGIN ---
          try {
            setLoading(true);
            // 1. Save token so axios interceptor can use it
            localStorage.setItem("authToken", token);

            // 2. We need the User object to complete the login state
            // (AuthContext requires both token and user to be isAuthenticated)
            ...
            // 4. Force a hard redirect to Dashboard to reload AuthContext
            // (Since AuthContext only checks localStorage on mount)
        ...
        <div className="relative z-10 flex flex-col justify-center px-12 py-16 text-white w-full max-w-2xl mx-auto">
          <div className="mb-10">
            ...
            <p className="text-lg text-indigo-100 leading-relaxed max-w-md">
              Streamline your Software Project Management Plan assessment with
              our automated analysis and grading system.
            </p>
          </div>
          ...
          <div className="grid grid-cols-3 gap-4 max-w-lg">
            {[
              ...
              <div
                key={i}
                className="flex flex-col items-center text-center p-4 rounded-xl bg-white/5 backdrop-blur-md border border-white/10 shadow-lg hover:bg-white/10 transition duration-300 transform hover:-translate-y-1"
              >
                <div
                  className={`w-12 h-12 rounded-lg bg-gradient-to-br ${item.gradient} flex items-center justify-center mb-3 shadow-inner`}
                >
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      ...
                      <input
                        ...
                        className="w-full px-4 py-3 rounded-lg border border-slate-200 focus:outline-none focus:ring-2 focus:ring-purple-500 bg-slate-50 focus:bg-white"
                        placeholder="John"
                        required
                      />
                    </div>
                  ...
                  <div>
                    ...
                    <div className="relative">
                      <select
                        ...
                        value={formData.role}
                        onChange={handleInputChange}
                        className="w-full px-4 py-3 rounded-lg border border-slate-200 focus:outline-none focus:ring-2 focus:ring-purple-500 bg-slate-50 focus:bg-white appearance-none cursor-pointer"
                      >
              ...
              <div>
                ...
                <div className="grid grid-cols-3 gap-3">
                  ...
                  <a
                    ...
                    className="flex items-center justify-center px-4 py-2.5 border border-slate-200 rounded-lg hover:bg-slate-50 transition-colors cursor-pointer"
                  >
                  ...
                  {/* Facebook - di pa mogana (Placeholder) */}
                  <button
                    type="button"
                  ...
                  {/* LinkedIn - di pa mogana (Placeholder) */}
                  <button
                    type="button"
                    className="flex items-center justify-center px-4 py-2.5 border border-slate-200 rounded-lg hover:bg-slate-50 transition-colors"
                  >
